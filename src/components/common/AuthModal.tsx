...
export const AuthModal: React.FC<AuthModalProps> = ({ onClose, initialTab = 'login', initialInviteToken = '' }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.name === 'AuthProvisioningError') {
          ...
          {provisionFailed ? (
            <div className="py-8 flex flex-col items-center justify-center space-y-4 animate-in fade-in zoom-in duration-300">
              ...
              <div className="text-center space-y-2">
                <h4 className="text-lg font-bold text-slate-800">Échec du provisionnement</h4>
                <p className="text-sm text-slate-600 max-w-[320px] mx-auto">
                  La création de votre espace de travail a échoué et n'a pas pu être réparée automatiquement.
                  Veuillez contacter le support ou recommencer avec une autre adresse email.
                </p>
              </div>
              <div className="flex gap-3 mt-6">
                <button 
                  onClick={() => { setProvisionFailed(false); setTab('register'); }} 
                  className="px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition-colors"
                >
                  Réessayer l'inscription
                </button>
                <button 
                  onClick={() => window.location.href = 'mailto:support@nexttransit.com'}
