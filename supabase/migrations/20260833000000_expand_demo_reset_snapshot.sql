-- =========================================================
-- EXPAND DEMO TENANT RESET: work_orders / driver_incidents / fleet_alerts /
-- inventory_items / cost_records
-- =========================================================
-- Prior reset_demo_tenant_data() (20260807000000, re-declared 20260827000000) DELETEd
-- fleet_alerts, work_orders, and driver_incidents on every reset but never re-inserted them —
-- demo_seed_snapshot only ever held 'vehicles', 'warranties', 'fuel_logs', 'telemetry_events'.
-- The very first scheduled reset would have silently emptied those three tables permanently.
-- This migration fixes that and additionally covers inventory_items and cost_records, so the
-- reset restores the full heterogeneous fleet dataset populated by
-- scripts/sync-demo-snapshot.ts (src/data/demoFleetDataset.ts).
...
DECLARE
    ...
    rec RECORD;
BEGIN
    -- Delete in FK-safe order (children before/alongside parents; ON DELETE CASCADE on
    -- vehicle_id makes exact ordering non-critical here, but explicit is clearer than relying
    -- on cascade for every table).
    ...
    -- Re-inject Inventory Items from Snapshot
    FOR rec IN SELECT jsonb_array_elements(snapshot_data) AS elem FROM public.demo_seed_snapshot WHERE table_name = 'inventory_items' LOOP
        INSERT INTO public.inventory_items (
            id, tenant_id, name, sku, quantity, reorder_threshold, unit_cost, compatible_vehicles, lead_time_days, category
        ) VALUES (
            (rec.elem->>'id')::uuid,
            DEMO_TENANT_ID,
            rec.elem->>'name',
            rec.elem->>'sku',
            COALESCE((rec.elem->>'quantity')::int, 0),
            COALESCE((rec.elem->>'reorder_threshold')::int, 5),
            ...
            COALESCE((rec.elem->>'lead_time_days')::int, 3),
            COALESCE(rec.elem->>'category', 'General')
        );
    END LOOP;

    -- Re-inject Driver Incidents from Snapshot (must precede work_orders: work_orders.related_incident_id references this table)
    FOR rec IN SELECT jsonb_array_elements(snapshot_data) AS elem FROM public.demo_seed_snapshot WHERE table_name = 'driver_incidents' LOOP
        INSERT INTO public.driver_incidents (
            id, tenant_id, vehicle_id, vehicle_plate, reported_by, category, description, matched_to_fault, related_fault_code, status, created_date
        ) VALUES (
            (rec.elem->>'id')::uuid,
            DEMO_TENANT_ID,
            (rec.elem->>'vehicle_id')::uuid,
            rec.elem->>'vehicle_plate',
            rec.elem->>'reported_by',
            rec.elem->>'category',
            rec.elem->>'description',
            COALESCE((rec.elem->>'matched_to_fault')::boolean, false),
            rec.elem->>'related_fault_code',
            COALESCE(rec.elem->>'status', 'Investigation'),
            COALESCE((rec.elem->>'created_date')::timestamptz, NOW())
        );
    END LOOP;

    -- Re-inject Work Orders from Snapshot
    FOR rec IN SELECT jsonb_array_elements(snapshot_data) AS elem FROM public.demo_seed_snapshot WHERE table_name = 'work_orders' LOOP
        INSERT INTO public.work_orders (
            id, tenant_id, vehicle_id, vehicle_plate, type, status, labor_hours, hourly_rate, labor_cost, parts_used, before_notes, after_notes, created_date, closed_date, assigned_mechanic_id, assigned_mechanic_name, related_fault_code, related_incident_id
        ) VALUES (
            (rec.elem->>'id')::uuid,
            DEMO_TENANT_ID,
            (rec.elem->>'vehicle_id')::uuid,
            rec.elem->>'vehicle_plate',
            COALESCE(rec.elem->>'type', 'Corrective'),
            COALESCE(rec.elem->>'status', 'Open'),
            COALESCE((rec.elem->>'labor_hours')::decimal, 0),
            COALESCE((rec.elem->>'hourly_rate')::decimal, 0),
            COALESCE((rec.elem->>'labor_cost')::decimal, 0),
            COALESCE(rec.elem->'parts_used', '[]'::jsonb),
            rec.elem->>'before_notes',
            rec.elem->>'after_notes',
            COALESCE((rec.elem->>'created_date')::timestamptz, NOW()),
            (rec.elem->>'closed_date')::timestamptz,
            rec.elem->>'assigned_mechanic_id',
            rec.elem->>'assigned_mechanic_name',
            rec.elem->>'related_fault_code',
            (rec.elem->>'related_incident_id')::uuid
        );
    END LOOP;

    -- Re-inject Fleet Alerts from Snapshot
    FOR rec IN SELECT jsonb_array_elements(snapshot_data) AS elem FROM public.demo_seed_snapshot WHERE table_name = 'fleet_alerts' LOOP
        INSERT INTO public.fleet_alerts (
            id, tenant_id, timestamp, rule_id, title, description, severity, vehicle_id, part_id, read
        ) VALUES (
            (rec.elem->>'id')::uuid,
            DEMO_TENANT_ID,
            COALESCE((rec.elem->>'timestamp')::timestamptz, NOW()),
            rec.elem->>'rule_id',
            rec.elem->>'title',
            rec.elem->>'description',
            rec.elem->>'severity',
            (rec.elem->>'vehicle_id')::uuid,
            (rec.elem->>'part_id')::uuid,
            COALESCE((rec.elem->>'read')::boolean, false)
        );
    END LOOP;

    -- Re-inject Cost Records from Snapshot
    FOR rec IN SELECT jsonb_array_elements(snapshot_data) AS elem FROM public.demo_seed_snapshot WHERE table_name = 'cost_records' LOOP
        INSERT INTO public.cost_records (
            id, tenant_id, vehicle_id, vehicle_plate, category, amount, budget_for_category, period, work_order_id, related_fault_code
        ) VALUES (
            (rec.elem->>'id')::uuid,
            DEMO_TENANT_ID,
            (rec.elem->>'vehicle_id')::uuid,
            rec.elem->>'vehicle_plate',
            rec.elem->>'category',
            COALESCE((rec.elem->>'amount')::decimal, 0),
            COALESCE((rec.elem->>'budget_for_category')::decimal, 0),
            rec.elem->>'period',
            (rec.elem->>'work_order_id')::uuid,
            rec.elem->>'related_fault_code'
        );
    END LOOP;
